import sys
from banco_dados.conexao import conectar_banco_nuvem, conecta_robo
...
class TelaMenu(QMainWindow, Ui_Menu_Principal):
    def __init__(self, id_user, parent=None):
        ...
        self.versao = f"Versão: 3.00.000"
        self.data_versao = f"Data Versão: 28/11/2024"
        self.id_usuario = id_user
        self.definir_usuario()
    ...
    def definir_botoes(self):
        ...
        try:
            cur = conecta.cursor()
            cur.execute(f"SELECT id, botoes FROM USUARIO_VERSAO "
                        f"where nome_pc = '{self.nome_computador}' "
                        f"and usuario_id = {self.id_usuario};")
            dados_usuario = cur.fetchall()
            if dados_usuario:
                botoes = dados_usuario[0][1]
                print(botoes)
    ...
    def definir_usuario(self):
        ...
        try:
            cur = conecta.cursor()
            cur.execute(f"SELECT id, login FROM USUARIO "
                        f"where id = {self.id_usuario};")
            dados_usuario = cur.fetchall()
            if dados_usuario:
                login = dados_usuario[0][1]
                msg = f"Usuário Logado: {login}"
                self.label_Usuario.setText(msg)
    ...
    def definir_tela_action(self):
        try:
            ...
            if sender == self.action_Pre_Incluir_2:
                from menu_produto.pre_incluir import TelaPreIncluir
                self.pre_incluir = TelaPreIncluir(self.id_usuario)
                self.pre_incluir.show()

            elif sender == self.action_Pre_Status_2:
                from menu_produto.pre_status import TelaPreStatus
                ...
                from menu_produto.prod_consulta import TelaProdutoConsultar
    ...
    def envia_email(self):
        ...
        try:
            ...
            cur = conecta.cursor()
            cur.execute(f"SELECT id, nome_pc, usuario_id "
                        f"FROM USUARIO_VERSAO  "
                        f"where nome_pc = '{self.nome_computador}' "
                        f"and usuario_id = {self.id_usuario};")
            select = cur.fetchall()
            ...
            if not select:
                ...
                cursor.execute(f"Insert into USUARIO_VERSAO (nome_pc, usuario_id, versao, atualizacao) "
                               f"values ('{self.nome_computador}', {self.id_usuario}, '{versao_app}', '{timestamp}');")
                conecta.commit()
    ...
    def salva_versao(self):
        ...
        try:
            ...
            cur = conecta.cursor()
            cur.execute(f"SELECT id, nome_pc, versao "
                        f"FROM USUARIO_VERSAO "
                        f"where nome_pc = '{self.nome_computador}' and usuario_id = {self.id_usuario};")
            select = cur.fetchall()

            if select:
                id_versao, nome_pc, versao = select[0]
                cursor = conecta.cursor()
                cursor.execute(f"UPDATE USUARIO_VERSAO "
