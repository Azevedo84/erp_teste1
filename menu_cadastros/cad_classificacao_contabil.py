...
from forms.tela_cad_classificacao import *
...
class TelaCadastroClassificacao(QMainWindow, Ui_MainWindow):
    ...
    def lanca_numero_id(self):
        try:
            num_id = proximo_registro_autoincremento("PRODUTO_CLASSIFICACAO")
    ...
    def excluir_cadastro(self):
        ...
        try:
            ...
            if not num_id:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"select id, criacao, descricao "
                               f"from PRODUTO_CLASSIFICACAO where id = {num_id};")
                conjunto = cursor.fetchall()

                cursor = conecta.cursor()
                cursor.execute(f"SELECT * "
                               f"FROM PRODUTO "
                               f"WHERE CLASSIFICACAO_PRODUTO_ID = {num_id};")
                produtos = cursor.fetchall()

                if not conjunto:
                    self.mensagem_alerta(f'O cadastro da Classificação Contábil Nº {num_id} não existe!')
                elif produtos:
                    self.mensagem_alerta(f'O cadastro da Classificação Contábil Nº {num_id} não pode ser excluído, '
                                         f'pois existem vinculos com produtos!')
                else:
                    msg = f'Tem certeza que deseja excluir a Classificação Contábil {nome}?'
                    if self.pergunta_confirmacao(msg):
                        cursor = conecta.cursor()
                        cursor.execute(f"DELETE FROM PRODUTO_CLASSIFICACAO WHERE id = {num_id};")

                        conecta.commit()

                        self.mensagem_alerta(f"Cadastro da Classificação Contábil {nome} foi excluído com Sucesso!")
        ...
        try:
            ...
            if conjuntos:
                ...
                if campos_atualizados:
                    msg = f'Deseja realmente atualizar o cadastro da Classificação Contábil?'
                    if self.pergunta_confirmacao(msg):
                        campos_update = ", ".join(campos_atualizados)

                        cursor.execute(f"UPDATE PRODUTO_CLASSIFICACAO SET {campos_update} "
                                       f"WHERE id = {num_id_b};")

                        conecta.commit()

                        msg = f'O cadastro da Classificação Contábil {descr_sem_acentos} foi atualizado com sucesso!'
                        self.mensagem_alerta(msg)

            else:
                msg = f'Deseja realmente cadastrar esta Classificação Contábil?'
                if self.pergunta_confirmacao(msg):
                    cursor = conecta.cursor()
                    cursor.execute(f"Insert into PRODUTO_CLASSIFICACAO (descricao) "
                                   f"values ('{descr_sem_acentos}');")

                    conecta.commit()

                    msg = f'O cadastro da Classificação Contábil {descr_sem_acentos} foi criado com sucesso!'
                    self.mensagem_alerta(msg)
...
if __name__ == '__main__':
    ...
    tela = TelaCadastroClassificacao()
    tela.show()
    qt.exec_()
