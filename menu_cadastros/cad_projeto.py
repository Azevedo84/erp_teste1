...
from forms.tela_cad_projeto import *
...
class TelaCadastroProjeto(QMainWindow, Ui_MainWindow):
    ...
    def manipula_dados_tabela(self):
        try:
            ...
            cursor.execute(f"select id, data_criacao, projeto "
                           f"from projeto "
                           f"where id <> 0 "
                           f"order by projeto;")
    ...
    def lanca_numero_id(self):
        try:
            definir_proximo_registro(self.line_Num, "id", "projeto")
    ...
    def procura_palavra(self):
        try:
            ...
            if not palavra_consulta:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"SELECT DISTINCT id, data_criacao, projeto "
                               f"FROM projeto "
                               f"WHERE projeto LIKE '%{palavra_maiuscula}%' and id <> 0 "
                               f"ORDER BY projeto;")
    ...
    def excluir_cadastro(self):
        try:
            ...
            if not num_id:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"select id, data_criacao, projeto "
                               f"from projeto where id = {num_id} and id <> 0 ;")
                tipo_mat = cursor.fetchall()

                cursor = conecta.cursor()
                cursor.execute(f"SELECT * "
                               f"FROM produto "
                               f"WHERE projeto = {num_id};")
                produtos = cursor.fetchall()

                if not tipo_mat:
                    self.mensagem_alerta(f'O cadastro do Projeto Nº {num_id} não existe!')
                elif produtos:
                    self.mensagem_alerta(f'O cadastro do Projeto Nº {num_id} não pode ser excluído, '
                                         f'pois existem vinculos com produtos!')
                else:
                    msg = f'Tem certeza que deseja excluir o Projeto {nome}?'
                    if self.pergunta_confirmacao(msg):
                        cursor = conecta.cursor()
                        cursor.execute(f"DELETE FROM projeto WHERE id = {num_id};")

                        conecta.commit()

                        self.mensagem_alerta(f"Cadastro do Projeto {nome} foi excluído com Sucesso!")
    ...
    def salvar_dados(self):
        try:
            ...
            cursor = conecta.cursor()
            cursor.execute(f"select id, data_criacao, projeto "
                           f"from projeto where id = {num_id};")
            ...
            if tipos:
                ...
                if descr_sem_acentos != descricao_b:
                    campos_atualizados.append(f"projeto = '{descr_sem_acentos}'")

                if campos_atualizados:
                    msg = f'Deseja realmente atualizar o cadastro do Projeto?'
                    if self.pergunta_confirmacao(msg):
                        campos_update = ", ".join(campos_atualizados)

                        cursor.execute(f"UPDATE projeto SET {campos_update} "
                                       f"WHERE id = {num_id_b};")

                        conecta.commit()

                        msg = f'O cadastro do Projeto {descr_sem_acentos} foi atualizado com sucesso!'
                        self.mensagem_alerta(msg)

            else:
                msg = f'Deseja realmente cadastrar este Projeto?'
                if self.pergunta_confirmacao(msg):
                    cursor = conecta.cursor()
                    cursor.execute(f"Insert into projeto (ID, projeto) "
                                   f"values (GEN_ID(GEN_projeto_ID,1), '{descr_sem_acentos}');")

                    conecta.commit()

                    msg = f'O cadastro do Projeto {descr_sem_acentos} foi criado com sucesso!'
                    self.mensagem_alerta(msg)
